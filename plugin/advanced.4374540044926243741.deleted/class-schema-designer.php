...
class SBSchemaDesigner {
	...
	public static function render_screen() {
		...
		echo '<div class="wrap"><h1>Schema Designer</h1>';
		echo '<div id="sb-schema-designer" data-palette=\'' . $palette_json . '\' data-schema=\'' . $schema_json . '\' data-rest=\'' . $rest_base . '\' data-nonce=\'' . $nonce . '\' data-slug=\'' . esc_attr( $slug ) . '\'>';
		// Inline styles for schema designer (safe - no JS curly braces)
		echo '<style>.sb-palette-item{padding:8px 12px;border:1.5px solid #e5e7eb;border-radius:6px;margin-bottom:6px;cursor:grab;background:#f9fafb;font-size:.85rem;transition:all .15s;user-select:none}.sb-palette-item:hover{border-color:#c9a84c;background:#fefce8}.sb-palette-item.dragging{opacity:.5}.sb-field-row{display:flex;align-items:center;gap:8px;padding:8px 10px;background:#fff;border:1.5px solid #e5e7eb;border-radius:6px;margin-bottom:6px}#sb-schema-canvas.drag-over{border-color:#c9a84c;background:#fefce8}</style>';
		...
		echo '</div>'; // end sb-schema-designer
	}
}
