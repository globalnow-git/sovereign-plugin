...
class SBVisualDesigner {
	...
	public static function render_screen(): void {
		...
		/* Graph canvas */
		#sb-vd-canvas { width:100%; min-height:500px; position:relative; background:#fff; border-radius:10px; border:1px solid #e5e7eb; overflow:auto; }
		svg#sb-graph-svg { width:100%; min-height:500px; }

		/* Node styles */
		.sb-node { cursor:pointer; }
		.sb-node rect { rx:8; ry:8; stroke-width:2; }
			...
			<div id="sb-vd-sidebar">
				...
				<div id="sb-vd-bp-list">
					...
					<div class="sb-vd-bp-item <?php echo $bp->id === $blueprint_id ? 'active' : ''; ?>"
						onclick="sbVDLoad(<?php echo (int) $bp->id; ?>, '<?php echo esc_js( $bp->label ); ?>')">
		...
		function sbVDLoad(id, label) {
			sbVDCurrentId    = id;
			sbVDCurrentLabel = label;
			document.getElementById('sb-vd-title').textContent = label;
			document.querySelectorAll('.sb-vd-bp-item').forEach(function(el){ el.classList.remove('active'); });
		...
		function sbVDRenderGraph(data) {
			...
			svg.setAttribute('viewBox', '0 0 ' + Math.max(maxX, 800) + ' ' + Math.max(maxY, 400));
			svg.style.minHeight = Math.max(maxY, 400) + 'px';
		}
