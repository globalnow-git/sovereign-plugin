...
PROMPT;
	...
	public static function on_form_submitted( string $signal_type, $value, int $user_id ): void {
		...
		// Find the campaign linked to the blueprint-builder blueprint
		...
		$campaign_id = (int) $wpdb->get_var( $wpdb->prepare(
			"SELECT id FROM {$wpdb->prefix}sb_campaigns WHERE blueprint_id = %d LIMIT 1",
			$bp_id
		) );
		...
		// Override campaign pipeline to blueprint-builder
		update_post_meta( $campaign_id, '_sb_pipeline_slug', self::PIPELINE_SLUG );

		SB_Factory_API::execute_pipeline( $campaign_id, $input_text );
	}
