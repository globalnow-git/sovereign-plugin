		...
		function sbVDOpenNode(node) {
			...
			switch(node.type) {
				case 'blueprint':
					...
					break;
				case 'form':
					// Look up form by slug to get ID
					fetch(sbVDRestBase + '/form/lookup?slug=' + encodeURIComponent(slug), {
						headers: { 'X-WP-Nonce': sbVDNonce }
					})
					.then(function(r){ return r.json(); })
					.then(function(data){
						if (data && data.id) {
							window.open(base + '?page=sb-tiny-forms&action=edit&id=' + data.id, '_blank');
						} else {
							window.open(base + '?page=sb-tiny-forms', '_blank');
						}
					});
					return;
				case 'schema':
				...
				case 'road':
					...
					break;
				case 'signal':
					// No dedicated signal page — go to submissions filtered view
					editUrl = base + '?page=sb-submissions';
					break;
				case 'page':
					// Look up WordPress page by slug
					fetch('<?php echo esc_js( get_rest_url( null, "wp/v2/pages" ) ); ?>?slug=' + encodeURIComponent(slug) + '&_fields=id', {
						headers: { 'X-WP-Nonce': sbVDNonce }
					})
					.then(function(r){ return r.json(); })
					.then(function(pages){
						if (pages && pages.length && pages[0].id) {
							window.open('<?php echo esc_js( admin_url( "post.php?action=edit&post=" ) ); ?>' + pages[0].id, '_blank');
						} else {
