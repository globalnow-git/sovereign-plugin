...
class SBSchemaDesigner {
	...
	public static function render_screen() {
		...
		(function(){
			...
			// Load existing schema fields if slug provided
			...
			function renderField(field) {
				...
				row.innerHTML =
					'<span class="sb-field-type">' + field.type + '</span>' +
					'<input type="text" class="sb-field-label-input" placeholder="Column label..." value="' + (field.label||'') + '" data-id="' + field.id + '">' +
					'<span class="sb-field-remove" data-id="' + field.id + '" title="Remove">✕</span>';
				canvas.appendChild(row);

				row.querySelector('.sb-field-label-input').addEventListener('input', function(e) {
					var f = fields.find(function(x){ return x.id === e.target.dataset.id; });
					if (f) { f.label = e.target.value; }
				});

				row.querySelector('.sb-field-remove').addEventListener('click', function(e) {
					var fid = e.target.dataset.id;
					fields = fields.filter(function(x){ return x.id !== fid; });
					document.getElementById('row_' + fid).remove();
			...
			function buildSchemaData() {
				return {
					...
					permissions: { list: 'manage_sovereign' }
				};
			}

			document.getElementById('sb-schema-save-draft').addEventListener('click', function() {
				...
				fetch(restBase + '/schema/draft', {
					...
					body: JSON.stringify({ slug: slug, schema: data })
				})
			...
			document.getElementById('sb-schema-preview').addEventListener('click', function() {
				var data = buildSchemaData();
				var pane = document.getElementById('sb-schema-preview-pane');
				pane.innerHTML = '<table style="width:100%;border-collapse:collapse;">' +
					'<thead><tr>' + data.columns.map(function(c){ return '<th style="text-align:left;padding:6px;border-bottom:2px solid #e5e7eb;font-size:0.78rem;color:#6b7280;text-transform:uppercase;">' + c.label + '</th>'; }).join('') + '</tr></thead>' +
					'<tbody><tr>' + data.columns.map(function(){ return '<td style="padding:6px;border-bottom:1px solid #f3f4f6;color:#9ca3af;font-size:0.78rem;">sample data</td>'; }).join('') + '</tr></tbody>' +
					'</table>';
			});

			document.getElementById('sb-schema-publish').addEventListener('click', function() {
