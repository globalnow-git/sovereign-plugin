...
class SBSchemaDesigner {
	...
	public static function render_screen() {
		...
		// Schema Designer JS - output directly
		$sd_nonce = wp_create_nonce('wp_rest');
		$sd_slug  = esc_js( $slug );
		$sd_rest  = esc_url_raw( get_rest_url( null, 'sovereign-builder/v1' ) );
		echo '<style>';
		echo '.sb-palette-item{padding:8px 12px;border:1.5px solid #e5e7eb;border-radius:6px;margin-bottom:6px;cursor:grab;background:#f9fafb;font-size:0.85rem;transition:all 0.15s;user-select:none;}';
		echo '.sb-palette-item:hover{border-color:#c9a84c;background:#fefce8;}';
		echo '.sb-palette-item.dragging{opacity:0.5;cursor:grabbing;}';
		echo '.sb-field-row{display:flex;align-items:center;gap:8px;padding:8px 10px;background:#fff;border:1.5px solid #e5e7eb;border-radius:6px;margin-bottom:6px;cursor:move;}';
		echo '.sb-field-row:hover{border-color:#c9a84c;}';
		echo '.sb-field-row input[type=text]{border:1px solid #e5e7eb;border-radius:4px;padding:3px 6px;font-size:0.82rem;width:140px;}';
		echo '.sb-field-remove{color:#dc2626;cursor:pointer;font-size:1rem;padding:0 4px;}';
		echo '#sb-schema-canvas.drag-over{border-color:#c9a84c;background:#fefce8;}';
		echo '</style>';
		echo '<script>';
		echo 'document.addEventListener("DOMContentLoaded",function(){';
		echo 'var canvas=document.getElementById("sb-schema-canvas");';
		echo 'var placeholder=document.getElementById("sb-canvas-placeholder");';
		echo 'var fields=[];';
		echo 'var restBase=' . wp_json_encode( $sd_rest ) . ';';
		echo 'var nonce=' . wp_json_encode( $sd_nonce ) . ';';
		echo 'var slug=' . wp_json_encode( $sd_slug ) . ';';
		echo 'var ex=JSON.parse(document.getElementById("sb-schema-designer").dataset.schema||"{}");';
		echo 'if(ex&&ex.columns&&ex.columns.length){ex.columns.forEach(function(c){addField("text",c.label||c.key||"");});}';
		echo 'else if(ex&&ex.fields&&ex.fields.length){ex.fields.forEach(function(f){addField(f.type||"text",f.label||f.key||"");});}';
		echo 'document.querySelectorAll(".sb-palette-item").forEach(function(item){';
		echo 'item.setAttribute("draggable","true");';
		echo 'item.addEventListener("dragstart",function(e){e.dataTransfer.setData("fieldType",item.dataset.type);item.classList.add("dragging");});';
		echo 'item.addEventListener("dragend",function(){item.classList.remove("dragging");});';
		echo 'item.addEventListener("click",function(){addField(item.dataset.type,"");});';
		echo '});';
		echo 'canvas.addEventListener("dragover",function(e){e.preventDefault();canvas.classList.add("drag-over");});';
		echo 'canvas.addEventListener("dragleave",function(){canvas.classList.remove("drag-over");});';
		echo 'canvas.addEventListener("drop",function(e){e.preventDefault();canvas.classList.remove("drag-over");var t=e.dataTransfer.getData("fieldType");if(t){addField(t,"");}});';
		echo 'function addField(type,label){if(placeholder){placeholder.style.display="none";}var id="f"+Date.now();var field={id:id,type:type,label:label||""};fields.push(field);renderField(field);}';
		echo 'function renderField(field){var row=document.createElement("div");row.className="sb-field-row";row.id="row_"+field.id;row.innerHTML="<span class=\'sb-field-type\'>"+field.type+"</span><input type=\'text\' placeholder=\'Column label...\' value=\'"+field.label+"\' data-id=\'"+field.id+"\'><span class=\'sb-field-remove\' data-id=\'"+field.id+"\'>&#x2715;</span>";canvas.appendChild(row);row.querySelector("input").addEventListener("input",function(e){var f=fields.find(function(x){return x.id===e.target.dataset.id;});if(f){f.label=e.target.value;}});row.querySelector(".sb-field-remove").addEventListener("click",function(e){var fid=e.target.dataset.id;fields=fields.filter(function(x){return x.id!==fid;});var r=document.getElementById("row_"+fid);if(r)r.remove();if(!fields.length&&placeholder){placeholder.style.display="";}});}';
		echo 'function buildData(){return{slug:slug,source_table:"sb_submissions",fields:fields.map(function(f){return{key:f.label.toLowerCase().replace(/\s+/g,"_")||f.id,label:f.label,type:f.type};}),columns:fields.map(function(f){return{key:f.label.toLowerCase().replace(/\s+/g,"_")||f.id,label:f.label};})};}';
		echo 'document.getElementById("sb-schema-save-draft").addEventListener("click",function(){if(!slug){alert("No slug.");return;}fetch(restBase+"/schema/draft",{method:"POST",headers:{"Content-Type":"application/json","X-WP-Nonce":nonce},body:JSON.stringify({slug:slug,schema:buildData()})}).then(function(r){return r.json();}).then(function(res){alert(res.success?"Saved!":"Failed: "+(res.error||""));}).catch(function(){alert("Save failed.");});});';
		echo 'document.getElementById("sb-schema-preview").addEventListener("click",function(){var d=buildData();var p=document.getElementById("sb-schema-preview-pane");p.innerHTML="<table style=\'width:100%;border-collapse:collapse\'><thead><tr>"+d.columns.map(function(c){return"<th style=\'text-align:left;padding:6px;border-bottom:2px solid #e5e7eb;font-size:0.78rem;color:#6b7280\'>"+c.label+"</th>";}).join("")+"</tr></thead><tbody><tr>"+d.columns.map(function(){return"<td style=\'padding:6px;color:#9ca3af;font-size:0.78rem\'>sample</td>";}).join("")+"</tr></tbody></table>";});';
		echo 'document.getElementById("sb-schema-publish").addEventListener("click",function(){if(!slug){alert("No slug.");return;}fetch(restBase+"/schema/publish",{method:"POST",headers:{"Content-Type":"application/json","X-WP-Nonce":nonce},body:JSON.stringify({slug:slug})}).then(function(r){return r.json();}).then(function(res){alert(res.success?"Published!":"Failed.");}).catch(function(){alert("Failed.");});});';
		echo '});';
		echo '</script>';
		echo '</div>';
	}
