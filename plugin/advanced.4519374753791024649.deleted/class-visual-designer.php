	...
	public static function render_screen(): void {
		...
		function sbVDOpenNode(node) {
			...
			switch(node.type) {
				...
				case 'signal':
					// Show signal activity in detail panel then open audit log
					sbVDShowSignalActivity(node);
					window.open(base + '?page=sb-addon-logs', '_blank');
					break;

				case 'page':
					fetch('<?php echo esc_js( get_rest_url( null, "wp/v2/pages" ) ); ?>?slug=' + encodeURIComponent(slug) + '&_fields=id&per_page=1', { headers: { 'X-WP-Nonce': sbVDNonce } })
					.then(function(r){ return r.json(); })
					.then(function(pages){
						var url = (pages && pages.length && pages[0].id)
							? '<?php echo esc_js( admin_url( "post.php?action=edit&post=" ) ); ?>' + pages[0].id
							: '<?php echo esc_js( admin_url( "edit.php?post_type=page" ) ); ?>';
						console.log('Page lookup result:', pages, 'URL:', url);
						window.open(url, '_blank');
					})
					.catch(function(){ window.open('<?php echo esc_js( admin_url( "edit.php?post_type=page" ) ); ?>', '_blank'); });
					break;
