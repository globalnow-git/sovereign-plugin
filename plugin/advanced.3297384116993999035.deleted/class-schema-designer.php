...
class SBSchemaDesigner {
	...
	public static function render_screen() {
		...
		$schema = $slug ? json_decode( ( function() use ( $slug ) {
			global $wpdb;
			$row = $wpdb->get_row( $wpdb->prepare( "SELECT schema_json FROM {$wpdb->prefix}sb_view_schemas WHERE slug = %s", $slug ) );
			return $row ? $row->schema_json : '{}';
		} )(), true ) : null;
