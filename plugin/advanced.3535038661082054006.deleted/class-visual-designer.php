		...
		function sbVDOpenNode(node) {
			...
			switch(node.type) {
				...
				case 'signal':
					...
					break;
				case 'page':
					// Look up WordPress page by slug and open its editor
					fetch('<?php echo esc_js( get_rest_url( null, "wp/v2/pages" ) ); ?>?slug=' + encodeURIComponent(slug) + '&_fields=id,slug,link', {
						headers: { 'X-WP-Nonce': sbVDNonce }
					})
					.then(function(r){ return r.json(); })
					.then(function(pages){
