	...
	public static function render_forms_screen(): void {
		...
		if ( $action === 'edit' && $edit_id ) {
			$form = $wpdb->get_row( $wpdb->prepare(
				"SELECT * FROM {$wpdb->prefix}sb_tiny_forms WHERE id = %d",
				$edit_id
			) );
			...
			?>
			<style>
			.sb-fe-wrap { max-width:900px; font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif; }
			.sb-fe-card { background:#fff; border-radius:10px; box-shadow:0 1px 4px rgba(0,0,0,.08); overflow:hidden; margin-bottom:1.5rem; }
			.sb-fe-head { background:#1a1a2e; padding:1rem 1.5rem; border-bottom:3px solid #c9a84c; display:flex; align-items:center; justify-content:space-between; }
			.sb-fe-head h2 { margin:0; font-size:1rem; font-weight:600; color:#f5f0e8; }
			.sb-fe-body { padding:1.5rem; }
			.sb-fe-meta { display:flex; gap:2rem; margin-bottom:1.5rem; }
			.sb-fe-meta-item { }
			.sb-fe-meta-label { font-size:0.72rem; font-weight:700; text-transform:uppercase; letter-spacing:0.05em; color:#6b7280; }
			.sb-fe-meta-value { font-size:0.92rem; font-weight:600; color:#1a1a2e; margin-top:0.2rem; }
			.sb-fe-field { padding:0.85rem 1rem; border:1.5px solid #e5e7eb; border-radius:8px; margin-bottom:0.75rem; background:#f9fafb; }
			.sb-fe-field-label { font-size:0.82rem; font-weight:700; color:#1a1a2e; text-transform:uppercase; letter-spacing:0.04em; }
			.sb-fe-field-meta { font-size:0.75rem; color:#6b7280; margin-top:0.2rem; }
			.sb-fe-field-key { font-family:monospace; background:#e5e7eb; padding:0.1rem 0.4rem; border-radius:3px; font-size:0.72rem; }
			.sb-badge { display:inline-flex; padding:0.15rem 0.55rem; border-radius:10px; font-size:0.72rem; font-weight:700; }
			.sb-badge-active { background:#ecfdf5; color:#059669; }
			.sb-badge-required { background:#fef2f2; color:#dc2626; }
			.sb-badge-optional { background:#f3f4f6; color:#6b7280; }
			</style>
			<div class="wrap sb-fe-wrap">
				<h1>
					<a href="<?php echo esc_url( admin_url( 'admin.php?page=sb-tiny-forms' ) ); ?>">Forms</a>
					→ <?php echo esc_html( $form->label ); ?>
				</h1>

				<div class="sb-fe-card">
					<div class="sb-fe-head">
						<h2><?php echo esc_html( $form->label ); ?></h2>
						<span class="sb-badge sb-badge-active"><?php echo esc_html( strtoupper( $form->status ) ); ?></span>
					</div>
					<div class="sb-fe-body">
						<div class="sb-fe-meta">
							<div class="sb-fe-meta-item">
								<div class="sb-fe-meta-label">Slug</div>
								<div class="sb-fe-meta-value"><code><?php echo esc_html( $form->slug ); ?></code></div>
							</div>
							<div class="sb-fe-meta-item">
								<div class="sb-fe-meta-label">Save Adapter</div>
								<div class="sb-fe-meta-value"><?php echo esc_html( $form->save_adapter ?? 'submission_table' ); ?></div>
							</div>
							<div class="sb-fe-meta-item">
								<div class="sb-fe-meta-label">Submissions</div>
								<div class="sb-fe-meta-value">
									<a href="<?php echo esc_url( admin_url( 'admin.php?page=sb-submissions&form=' . $form->slug ) ); ?>"><?php echo $sub_count; ?> submission<?php echo $sub_count !== 1 ? 's' : ''; ?></a>
								</div>
							</div>
							<div class="sb-fe-meta-item">
								<div class="sb-fe-meta-label">Fields</div>
								<div class="sb-fe-meta-value"><?php echo count( $fields ); ?> fields</div>
							</div>
						</div>

						<h3 style="margin:0 0 1rem;color:#1a1a2e;font-size:0.9rem;border-bottom:1px solid #f3f4f6;padding-bottom:0.5rem;">Form Fields</h3>
						<?php foreach ( $fields as $field ) : ?>
						<div class="sb-fe-field">
							<div style="display:flex;align-items:center;gap:0.75rem;">
								<span class="sb-fe-field-label"><?php echo esc_html( $field['label'] ?? $field['key'] ?? '' ); ?></span>
								<span class="sb-fe-field-key"><?php echo esc_html( $field['key'] ?? '' ); ?></span>
								<span class="sb-badge <?php echo ( $field['required'] ?? false ) ? 'sb-badge-required' : 'sb-badge-optional'; ?>">
									<?php echo ( $field['required'] ?? false ) ? 'Required' : 'Optional'; ?>
								</span>
							</div>
							<div class="sb-fe-field-meta">
								Type: <strong><?php echo esc_html( $field['type'] ?? 'text' ); ?></strong>
								<?php if ( ! empty( $field['placeholder'] ) ) : ?>
								· Placeholder: <?php echo esc_html( $field['placeholder'] ); ?>
								<?php endif; ?>
								<?php if ( ! empty( $field['options'] ) ) : ?>
								· Options: <?php echo esc_html( implode( ', ', $field['options'] ) ); ?>
								<?php endif; ?>
							</div>
						</div>
						<?php endforeach; ?>
						...
						<div style="margin-top:1.5rem;padding-top:1rem;border-top:1px solid #f3f4f6;">
							<a href="<?php echo esc_url( admin_url( 'admin.php?page=sb-tiny-forms' ) ); ?>" class="button">← Back to Forms</a>
							<a href="<?php echo esc_url( admin_url( 'admin.php?page=sb-submissions&form=' . $form->slug ) ); ?>" class="button button-primary" style="margin-left:0.5rem;">View Submissions</a>
							<?php
							\$pages_edit = get_posts( [ 'post_type' => 'page', 'post_status' => 'publish', 'posts_per_page' => 1, 's' => \$form->slug ] );
							if ( \$pages_edit ) :
							?>
							<a href="<?php echo esc_url( admin_url( 'post.php?post=' . $pages_edit[0]->ID . '&action=edit' ) ); ?>" class="button" style="margin-left:0.5rem;">Edit Page</a>
							<a href="<?php echo esc_url( get_permalink( $pages_edit[0]->ID ) ); ?>" class="button" target="_blank" style="margin-left:0.5rem;">View Form ↗</a>
							<?php endif; ?>
						</div>
					</div>
				</div>
