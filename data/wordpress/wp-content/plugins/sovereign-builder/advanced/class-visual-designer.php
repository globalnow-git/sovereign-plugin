				...
				case 'page':
					...
					.then(function(pages){
						...
						if (pages && pages.length && pages[0].id) {
							var url = '<?php echo esc_url_raw( admin_url( "post.php" ) ); ?>?post=' + pages[0].id + '&action=edit';
							console.log('Opening:', url, 'for page:', pages[0].title);
							window.open(url, '_blank');
						} else {
							window.open('<?php echo esc_url_raw( admin_url( "edit.php?post_type=page" ) ); ?>', '_blank');
						}
					})
					.catch(function(e){ console.error('Page lookup failed:', e); window.open('<?php echo esc_url_raw( admin_url( "edit.php?post_type=page" ) ); ?>', '_blank'); });
					break;
